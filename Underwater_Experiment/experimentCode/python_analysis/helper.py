...
def parzen_kernel(xi, x, var):
    result = (1 / (sqrt(2 * pi * var))) * exp((-1/2)*(pow(x - xi, 2) / var))
    return result

def std_norm_kernel(xi, x):
    numer = np.exp(-0.5*np.power((xi - x),2))
    denom = 1/(np.power(2 * np.pi, 0.5))
    return numer / denom

def parzen_mean(x, fx, granularity):
    dx = 1 / granularity
    #print( 'x is: ' + str(x) + ' fx is: ' + str(fx) + ' dx is: ' + str(dx))
    return x * fx * dx

def parzen_variance(x, fx, granularity, mean):
    dx = 1 / granularity
    return pow(x-mean, 2) * fx *dx

def parzen_window(X, granularity=100, var=1):
    '''Given a vector, run the Parzen window function with a Gaussian
       kernel.'''
    minimum = 0
    maximum = 0
    fx = 0
    minimum = np.min(X)  # Get something smaller and something larger so that I am well outside of the actual range
    maximum = np.max(X) #Trying to push some room.
    #print('The min is' + str(minimum))
    x = np.linspace(minimum, maximum, num=granularity)
    pdf_est = np.zeros(len(x))
    #print('range of interpolation.')
    #print(x)
    for i in range(0,len(x)):
        fx = 0
        for j in range(0,len(X)):
            #fx += parzen_kernel(X[j], x[i], var)
            fx += std_norm_kernel(X[j],x[i])
        fx= fx / len(X)
        pdf_est[i] = fx
    mean = 0
    variance = 0
    for a in range(0,len(pdf_est)):
        mean += parzen_mean(x[a], pdf_est[a], len(pdf_est))
    for a in range(0,len(pdf_est)):
        variance += parzen_variance(x[a], pdf_est[a], granularity, mean)
    return pdf_est, x, mean, variance

#I'm going to try a cheap and dirty Parzen Estimation
def kern(x):
    #Send in here a fully bandwidth adjusted vector.
    numer = np.exp(-(np.power(x,2)/2.0))
    denom = np.sqrt(2*np.pi)
    return numer / denom
    
def kdens(x, X):
    #x is one sample from all samples of X
    n = len(X)
    sigma = np.std(X)
    #let's get Silverman's bandwidth rule in action
...
def parzen(X, granularity=1000):
    #For now we keep it simple.
    n = len(X)
    #print('length is: ' + str(n))
    estimated = np.zeros(n) #This is the estimated PDF
    for i in range(n):
